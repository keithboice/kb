...
module.exports = {
	...
	// with a single module moduleNameMapper: {}, An array of regexp pattern strings, matched against all module paths
	// before considered 'visible' to the module loader modulePathIgnorePatterns: [], Activates notifications for test
	// results notify: false, An enum that specifies notification mode. Requires { notify: true } notifyMode:
	// "failure-change", A preset that is used as a base for Jest's configuration preset: undefined, Run tests from one
	// or more projects projects: undefined, Use this configuration option to add custom reporters to Jest reporters:
	// undefined, Automatically reset mock state between every test resetMocks: false, Reset the module registry before
	// running each individual test resetModules: false, A path to a custom resolver resolver: undefined, Automatically
	// restore mock state between every test restoreMocks: false,
	...
	watchPlugins: [ [ 'jest-watch-directories', { directories: [ 'packages', 'example', 'frontend', 'backend' ] } ] ]
};
